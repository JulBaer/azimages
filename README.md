...
- `type`: Type of experiment. User variable. For the original: sAIP = synthatic homologous AIP. mAIP: mixed homologous and heterologous AIP. short: quick framerate needed for successful DeLTA tracking
...
The variables are described in the script and an example json file is included.
...
The `masterdir` also needs to contain a meta file (add name in config `metacsv`), see  `.../MM_pipeline/meta_processing.csv` for an example and details below.
...
Model is stored in `.../stardist_models/mm` 
...
- `configpath` Path to the storage of the DeLTA config file. Edit your local copy of it: It needs to point to the tracking model (hdf5) file downloadable from Zenodo (10.5281/zenodo.10694025).
...
Visually aided manual image rotation detection to write that information in the meta csv file. The next script relies on pretty accurate vertical alignment of the chamber with the two connected chambers on top of each other.
