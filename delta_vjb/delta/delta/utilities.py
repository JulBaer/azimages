            ...
            for c, cha in enumerate(channels):
                for f, fra in enumerate(frames):
                    ...
                    if resize is not None:
                        ...
                        old_image_height, old_image_width = frame.shape
                        if resize[0]<old_image_height:
                            dif = old_image_height - new_image_height
                            crop_img = frame[dif:old_image_height, :]
                        else:
                            crop_img = frame
                        old_image_height, old_image_width = crop_img.shape
                        color = 0
                        result = np.full((resize[0],resize[1]), color, dtype=np.float32)
                        # compute center offset
                        x_center = (resize[0] - old_image_width) // 2
                        y_center = (resize[1] - old_image_height) // 2
                        # copy img image into center of result image
                        result[y_center:y_center+old_image_height, x_center:x_center+old_image_width] = crop_img
                        frame = result
